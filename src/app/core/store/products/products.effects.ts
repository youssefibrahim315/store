...
import { ProductService } from '../../../modules/product-management/services/product.service';
...
export class ProductEffectService {
  ...
  getProductsEffect$ = createEffect(() =>
    this.actions$.pipe(
      ...
      exhaustMap(([action]) => {
        const { currentPage,itemsPerPage } = action;
        let params = { currentPage,itemsPerPage };
        return this.productService.getList(params).pipe(
          map((res: any) =>{
